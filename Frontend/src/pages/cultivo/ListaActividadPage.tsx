...
} from "@/hooks/cultivo/useActividad";
...
const ListaActividadPage: React.FC = () => {
  ...
  const [selectedActividad, setSelectedActividad] = useState<ActividadForm | null>(null);
  ...
  const [finalizeData, setFinalizeData] = useState({
    ...
    herramientas: [] as { id: number; herramienta_id: number; nombre: string; cantidad_entregada: number; cantidad_devuelta: number; devuelta: boolean; fecha_devolucion: string }[],
    insumos: [] as { id: number; insumo_id: number; nombre: string; cantidad_usada: number; cantidad_devuelta: number }[],
  });
  ...
  const handleEdit = (actividad: ActividadForm) => {
    setSelectedActividad(actividad);
    setActividad({
      ...actividad,
      usuarios: actividad.usuarios_data?.map((u) => u.id) || [],
      insumos: actividad.prestamos_insumos?.map((i) => ({
        insumo_id: i.insumo_id,
        cantidad_usada: i.cantidad_usada,
      })) || [],
      herramientas: actividad.prestamos_herramientas?.map((h) => ({
  ...
  const handleDelete = (actividad: ActividadForm) => {
    setSelectedActividad(actividad);
    setIsDeleteModalOpen(true);
  };

  const handleFinalize = (actividad: ActividadForm) => {
    setSelectedActividad(actividad);
    const herramientasData = actividad.prestamos_herramientas?.map((h) => ({
      id: h.id,
      herramienta_id: h.herramienta_id,
      nombre: h.herramienta_nombre || h.herramienta?.nombre || "Herramienta",
    ...
    const insumosData = actividad.prestamos_insumos?.map((i) => ({
      id: i.id,
      insumo_id: i.insumo_id,
      nombre: i.insumo_nombre || i.insumo?.nombre || "Insumo",
  ...
  const transformedData = (actividades ?? []).map((actividad) => {
    ...
    const herramientasInfo =
      actividad.prestamos_herramientas?.map(
        (ph) =>
          `${ph.herramienta_nombre || ph.herramienta?.nombre || "Herramienta"} (${
            ph.cantidad_entregada || 1
          } ${ph.devuelta ? "devueltas" : "prestadas"})`
      ).join(", ") || "No se usaron herramientas";
    const insumosInfo =
      actividad.prestamos_insumos?.map(
        (pi) =>
          `${pi.insumo_nombre || pi.insumo?.nombre || "Insumo"} (${
            pi.cantidad_usada || 0
          }${pi.unidad_medida || pi.insumo?.unidad_medida?.abreviatura || ""})`
      ...
      acciones: (
        <>
          {actividad.estado !== "COMPLETADA" && (
            <button className="text-blue-500 hover:underline mr-2" onClick={() => handleFinalize(actividad)}>
              <CheckCircleIcon size={22} color="green" />
            </button>
          )}
          <button className="text-green-500 hover:underline mr-2" onClick={() => handleEdit(actividad)}>
            <EditIcon size={22} color="black" />
          </button>
          <button className="text-red-500 hover:underline" onClick={() => handleDelete(actividad)}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setActividad({ ...actividad, estado: e.target.value as ActividadForm["estado"] })}
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setActividad({ ...actividad, prioridad: e.target.value as ActividadForm["prioridad"] })}
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
          ...
          <div>
            ...
            <select
              ...
              value={actividad.tipo_actividad_id || 0}
              onChange={(e) => setActividad({ ...actividad, tipo_actividad_id: parseInt(e.target.value) })}
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
          ...
          <div>
            ...
            <select
              name="cultivo_id"
              value={actividad.cultivo_id || 0}
              onChange={(e) => setActividad({ ...actividad, cultivo_id: parseInt(e.target.value) })}
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
            ...
            <div className="space-y-2">
                ...
                <div key={index} className="flex gap-2 items-center">
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      newInsumos[index].insumo_id = parseInt(e.target.value);
              ...
              <button
                onClick={() =>
                  setActividad({
                    ...actividad,
                    insumos: [...(actividad.insumos || []), { insumo_id: 0, cantidad_usada: 0 }],
                  })
                }
            ...
            <div className="space-y-2">
              {actividad.herramientas?.map((herramienta, index) => (
                <div key={index} className="flex gap-2 items-center">
                  <select
                    className="flex-1 px-4 py-2 border border-gray-300 rounded-lg"
                    value={herramienta.herramienta_id}
                    onChange={(e) => {
                      const newHerramientas = [...(actividad.herramientas || [])];
                      newHerramientas[index].herramienta_id = parseInt(e.target.value);
                ...
                onClick={() =>
                  setActividad({
                    ...
                    herramientas: [
                      ...
                      { herramienta_id: 0, cantidad_entregada: 1, entregada: true, devuelta: false, fecha_devolucion: null },
                    ],
                  })
                }
                className="text-blue-500 text-sm"
